...
| **Last updated** | 2026-08-31 (session 5) |
...
| **Next action** | **FULL LOOP VERIFIED LIVE (session 6). Next: videos + docs.** spec → ghost `TASK.md` (4 subs, no real names) → consultancy (real Claude, ghost-only, **8 files, 7/7 ghost tests + build green**) → `client-agent open-real-pr` → `ghostc/real/001-add-companyx-integration`: 8 files/13 hunks, **leak-scan clean (0 ghost aliases)**, real names restored, **7/7 real tests + build green**, 0 fallbacks. Enablers: **(C2/C3)** `consultancy_agent/agent.py` — ordered `_SYSTEM`, `_MAX_STEPS` 24→40, `done:true` refused until `tests_green && build_green`, `agent.run()` records authoritative `ghost_tests`/`ghost_build` → metrics + `await_consultancy` merge. **(reverse)** `ghostc/patch.py::reverse_apply` — replays the consultancy's hunks onto the real pre-image (context verbatim from real, only `+` lines translated; forward `compile` is line-preserving); `reverse_pr.py` writes files directly, no `git apply` gate; `_translate` pass 1b handles `Vendor A`/`VENDOR A`. `pytest` green. Resume block + demo caveats: `SESSION_TODO.md` (offline stub can't demo the reverse; no `scripts/demo-companyx.sh` yet). Then: migrate the **full** pipeline to the same real-repo model → D (hook posts an event, graph `interrupt()`/resume) → E (docker-compose) → F (`run-eval-suite`, fix `patch.py` `reverse-compiler`→`reverse_compiler`). |
