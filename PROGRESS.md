...
| **Next action** | **C2 finish: harden `consultancy_agent/agent.py::_agent_loop` so Claude actually completes the CompanyX ticket** (live run hit `_MAX_STEPS=24` with ~4/6 ACs, never ran the tests). Options: bigger budget + an "AC checklist / not done, run the tests" nudge (accept `done` only after `run_tests`+`run_build` green), or add native Anthropic tool-use to `bridge/llm.py`. Success bar: `../ghostc-demo/ghost` on `ghostc/task/001-add-second-provider` passes `node --test` + `node scripts/build.js`. Then **C3:** record `ghost_tests`/`ghost_build` in `agent.metrics` + a `scripts/demo-companyx.sh`. Full checklist + how-to-resume: `SESSION_TODO.md`. Then: migrate the **full** pipeline to the same real-repo model → D (hook posts an event, graph `interrupt()`/resume) → E (docker-compose) → F (`run-eval-suite`, fix `patch.py` `reverse-compiler`→`reverse_compiler`). |
