...
| **Last updated** | 2026-09-01 (session 10 — the outbound screen: deterministic scoring + a client-side LLM adjudicator) |
...
ghostc screen  --text <ghost TASK.md> --candidates workspace/private/candidates.jsonl  # WORKS: scores the outbound text for UNCONFIGURED entities; exit 1 on a finding
...
pytest -q                                       # WORKS: 369 passed, 2 skipped (fixture built); parity from a clean checkout (more skips, 0 fails)
```
...
All eight commands are implemented — no stubs left.
...
### screen — what it does (session 10)

`compile` / `compile-spec` are **closed-world**: they substitute the entities
`privacy.yaml` + `mapping.json` name, and their fail-closed leak scan looks for those
same real spellings. A partner nobody configured is invisible to the redactor *and* to
its own gate. `screen` is the open-world second gate — it scores the compiler's
**output**, so every finding is by construction something the closed world missed.

Three evidence layers, folded into the same noisy-OR + `classify` as `discover`:

| layer | where | catches |
|---|---|---|
| structural shapes (`detect/shapes.py`) | `ghostc/screen.py` | secrets, internal hosts, contract/tenant ids, scoped packages, emails, RFC1918 |
| standing `discover` proposals | `ghostc/screen.py` | a name the scorer already flagged and nobody froze into the config |
| LLM adjudicator | `client_agent/screen_llm.py` | a partner's name in an English sentence — no anchor, so the detector is blind to it by design |

The model is a **recall input to a deterministic decision**: every claim is re-anchored
into the outbound text with `anchored_scan` (a name that exists only in the real half of
its prompt is dropped and counted), its signal is capped at 0.60, and no screen signal is
a *hard* signal — so `classify` can only ever return `review` / `ignore` here. The screen
queues and blocks; it never transforms anything.

Worked example — `specs/002-onboard-halcyon-cargo.md` (a ticket that exists to be blocked):

```
deterministic only          + adjudicator (Opus 5)
hal_live_9f8e…      0.55    0.81   shape + llm
HAL-CF-2026-01      0.45    0.76   shape + llm
gw.prod.halcyon…    0.45    0.77   shape + llm
dispatch.lead@…     0.35    0.70   shape + llm
                            0.58   Halcyon Freight    llm only
                            0.51   HalcyonClient      llm only
                            0.45   halcyonClient.js   llm only
```

On the clean `specs/001` spec, both layers report **0** findings. `--screen-mode
block|warn|off`, `--screen-llm best-effort|required|off` (best-effort records
`screen_llm: skipped` when no client key resolves — the deterministic layer still gates,
so offline CI stays green). A reviewer `ignore` in `decisions.jsonl` suppresses a surface
permanently; `write_findings` emits the `Candidate` shape, so `ghostc-review --candidates
...
| 2026-09-01 | **Screen the compiler's *output*, not its input** | Everything the closed world handled is already gone by then, so every finding is by construction an unknown entity. Screening the input would mostly re-report `privacy.yaml`. |
| 2026-09-01 | **The screen's LLM may accuse, never decide** (claims re-anchored in the outbound text; signal capped at 0.60; no screen signal is a *hard* signal) | A model that can be talked into clearing something is not a gate. The deterministic layer owns the decision; the model buys recall on prose the anchor-driven detector cannot see. |
| 2026-09-01 | The screen adjudicator is given the **real task and the ghost task** (user call) | Much higher recall — it can diff the two — and it widens a crossing the PR-consistency gate already makes rather than opening a new one. `role="client"` LLMs see inside the boundary; recorded in `THREAT_MODEL.md`. |
| 2026-09-01 | `--screen-llm best-effort` is the **default**, `required` is the production setting | Failing closed on an *unavailable* model would break offline CI and every `--backend stub` run for a model that was never going to be there. The gate degrades to deterministic-only and says so in the metrics rather than silently. |
| 2026-09-01 | A **structural** hit at a `restricted` level is queued even below `review_threshold` | The shape weights are tuned for a repo, where an email is usually a package author; in an outbound ticket it is a person, and `privacy.yaml` already says `restricted` needs a human. Deliberately not extended to the adjudicator — a shape is a fact, a model claim is an opinion. |
...
task text
   ...
   -> [Privacy Compiler]  deterministic tree-sitter JS/TS + HCL  -> ghost repo + ghost spec
        (closed world: substitutes only what privacy.yaml + the mapping name)
   -> [Outbound screen]  scores the compiled output for entities NOBODY configured:
        structural shapes + standing `discover` proposals + a client-side LLM
        adjudicator (accuses only; every claim re-anchored in the outbound text)
        -> BLOCK (fail closed) / human review queue
